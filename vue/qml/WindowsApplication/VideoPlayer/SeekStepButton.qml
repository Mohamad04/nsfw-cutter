...
Button {
    ...
    property int chevronCount: 2
    property bool lightMode: false
    property color iconColor: root.lightMode ? "#0F3B73" : "#F8FBFF"
    property string accessibilityLabel: ""
    readonly property bool backward: root.direction === "backward"
    readonly property bool compactStep: root.seconds === 1 && root.chevronCount === 1

    implicitWidth: root.compactStep ? 64 : 84
    implicitHeight: 36
    hoverEnabled: true
    padding: 0
    leftPadding: 0
    ...
    contentItem: Canvas {
        ...
        property int secondsState: root.seconds
        property int chevronCountState: root.chevronCount
        property bool backwardState: root.backward
        property bool compactStepState: root.compactStep
        ...
        onSecondsStateChanged: requestPaint()
        onChevronCountStateChanged: requestPaint()
        onBackwardStateChanged: requestPaint()
        onCompactStepStateChanged: requestPaint()
        onWidthChanged: requestPaint()
        onHeightChanged: requestPaint()
        ...
        function directionChevron(ctx, centerX, y1, y2, y3, spread, scale, offsetX, offsetY) {
            if (root.backward) {
                iconCanvas.chevron(ctx, centerX + spread, y1, centerX - spread, y2, centerX + spread, y3, scale, offsetX, offsetY)
            } else {
                iconCanvas.chevron(ctx, centerX - spread, y1, centerX + spread, y2, centerX - spread, y3, scale, offsetX, offsetY)
            }
        }
        ...
        function drawBadge(ctx, x, y, scale, offsetX, offsetY, active, light) {
            iconCanvas.roundedRect(ctx, x, y, 20.4, 12.6, 4.3, scale, offsetX, offsetY)
        ...
        function drawOneSecond(ctx, scale, offsetX, offsetY, active, light) {
            ctx.strokeStyle = light ? (active ? "rgba(37, 99, 235, 0.14)" : "rgba(37, 99, 235, 0.07)") : (active ? "rgba(88, 192, 255, 0.34)" : "rgba(88, 192, 255, 0.12)")
            ctx.lineWidth = (light ? 6.6 : 7.8) * scale
            iconCanvas.directionChevron(ctx, 21.5, 10.4, 18, 25.6, 5.8, scale, offsetX, offsetY)

            ctx.strokeStyle = iconCanvas.iconColorState
            ctx.lineWidth = 4.35 * scale
            iconCanvas.directionChevron(ctx, 21.5, 10.4, 18, 25.6, 5.8, scale, offsetX, offsetY)

            ctx.fillStyle = iconCanvas.iconColorState
            ctx.font = "800 " + Math.round(14.4 * scale) + "px sans-serif"
            ctx.textAlign = "left"
            ctx.textBaseline = "middle"
            ctx.fillText("1s", iconCanvas.mapX(35.0, scale, offsetX), iconCanvas.mapY(18.8, scale, offsetY))
        }

        function drawFiveSecond(ctx, scale, offsetX, offsetY, active, light) {
            if (root.backward) {
                iconCanvas.drawBadge(ctx, 10.8, 12.2, scale, offsetX, offsetY, active, light)

                var streakBack = ctx.createLinearGradient(iconCanvas.mapX(28.6, scale, offsetX), 0,
                                                          iconCanvas.mapX(33.8, scale, offsetX), 0)
                streakBack.addColorStop(0.0, light ? "rgba(37, 99, 235, 0.22)" : "rgba(255, 255, 255, 0.82)")
                streakBack.addColorStop(0.45, light ? (active ? "rgba(37, 99, 235, 0.20)" : "rgba(37, 99, 235, 0.10)") : (active ? "rgba(115, 218, 255, 0.78)" : "rgba(88, 190, 255, 0.32)"))
                streakBack.addColorStop(1.0, light ? "rgba(37, 99, 235, 0.0)" : "rgba(91, 190, 255, 0.0)")
                ctx.strokeStyle = streakBack
                ctx.lineWidth = 1.4 * scale
                ctx.beginPath()
                ctx.moveTo(iconCanvas.mapX(28.6, scale, offsetX), iconCanvas.mapY(12.2, scale, offsetY))
                ctx.lineTo(iconCanvas.mapX(33.8, scale, offsetX), iconCanvas.mapY(12.2, scale, offsetY))
                ctx.stroke()
                ...
                ctx.textBaseline = "middle"
                ctx.fillText("5", iconCanvas.mapX(21.0, scale, offsetX), iconCanvas.mapY(18.9, scale, offsetY))

                ctx.strokeStyle = light ? (active ? "rgba(37, 99, 235, 0.14)" : "rgba(37, 99, 235, 0.07)") : (active ? "rgba(88, 192, 255, 0.34)" : "rgba(88, 192, 255, 0.12)")
                ctx.lineWidth = (light ? 7.4 : 8.6) * scale
                iconCanvas.chevron(ctx, 47.8, 10.4, 37.0, 18, 47.8, 25.6, scale, offsetX, offsetY)
                iconCanvas.chevron(ctx, 60.8, 10.4, 50.0, 18, 60.8, 25.6, scale, offsetX, offsetY)
            ...
            iconCanvas.drawBadge(ctx, 52.8, 12.2, scale, offsetX, offsetY, active, light)

            var streakForward = ctx.createLinearGradient(iconCanvas.mapX(50.2, scale, offsetX), 0,
                                                         iconCanvas.mapX(55.4, scale, offsetX), 0)
            streakForward.addColorStop(0.0, light ? "rgba(37, 99, 235, 0.0)" : "rgba(91, 190, 255, 0.0)")
            streakForward.addColorStop(0.55, light ? (active ? "rgba(37, 99, 235, 0.20)" : "rgba(37, 99, 235, 0.10)") : (active ? "rgba(115, 218, 255, 0.78)" : "rgba(88, 190, 255, 0.32)"))
            streakForward.addColorStop(1.0, light ? "rgba(37, 99, 235, 0.22)" : "rgba(255, 255, 255, 0.82)")
            ctx.strokeStyle = streakForward
            ctx.lineWidth = 1.4 * scale
            ctx.beginPath()
        ...
        onPaint: {
            ...
            var baseWidth = root.compactStep ? 64 : 84
            var scale = Math.min(w / baseWidth, h / 36)
            var offsetX = (w - baseWidth * scale) / 2
            ...
            if (root.compactStep) {
                iconCanvas.drawOneSecond(ctx, scale, offsetX, offsetY, active, light)
            } else {
                iconCanvas.drawFiveSecond(ctx, scale, offsetX, offsetY, active, light)
            }
